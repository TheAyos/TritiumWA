...
exports.desc = '*[DEV]* Reloads a specific command.';
exports.usage = `.prefix${this.name} <command>`;
exports.example = `.prefix${this.name} help`;

exports.needArgs = false; // cuz it's a DEV command, will get removed

exports.run = function (client, message, args) {
    try {

        console.log(`[INFO] ${message.sender.id} requested reload`)
        if (message.sender.id !== client.config.youb_id) return client.reply(message.from, "Only creator can do this.", message.id);
