        ...
        async function love(array) {
            ...
            if (array.length > 2) {
                ...
                if (result < 25) {
                    ...
                    reactionGif = await fs.readFileSync(`./assets/love/crying${Math.floor(Math.random() * 4) + 1}.gif`, { encoding: 'base64' });
                } else if (result < 50) {
                    emoji = '❤️';
                    advice = 'It\'s below average, you have to find someone else.'
                    reactionGif = await fs.readFileSync(`./assets/love/nope${Math.floor(Math.random() * 5) + 1}.gif`, { encoding: 'base64' });
                } else if (result < 75) {
                    emoji = '💗';
                    advice = 'It\'s a great score!';
                    reactionGif = await fs.readFileSync(`./assets/love/neutral${Math.floor(Math.random() * 4) + 1}.gif`, { encoding: 'base64' });
                } else if (result <= 100) {
                    emoji = '💞';
                    advice = 'A relationship is possible !';
                    reactionGif = await fs.readFileSync(`./assets/love/happy${Math.floor(Math.random() * 5) + 1}.gif`, { encoding: 'base64' });
                }
            }
            ...
            await client.sendImageAsSticker(message.from, `data:image/gif;base64,${reactionGif}`);
