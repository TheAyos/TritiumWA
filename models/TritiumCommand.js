...
module.exports = class TritiumCommand {
  ...
  constructor(_run, props) {
    ...
    this.props.clientPerms =
      typeof this.props.clientPerms === "string" ? [this.props.clientPerms] : this.props.clientPerms;
  ...
  async run({ Tritium, msg, args, cleanArgs, chatPrefix, usedAlias, updateCooldowns }) {
    ...
    if (this.props.groupOnly && !msg.isGroupMsg) {
      return Tritium.reply(
        msg.from,
        `😿 Sorry ${msg.sender.pushname}, this command is only available in groups !`,
        msg.id,
        true,
      );
    }
    ...
    const isAdmin = msg.isGroupMsg
      ? msg.chat.groupMetadata.participants.find((u) => u.id === msg.sender.id && u.isAdmin) ||
        msg.sender.id === Tritium.config.youb_id
      : undefined;
    if (this.props.userPerms && this.props.userPerms.includes("ADMINISTRATOR") && !isAdmin) {
      return Tritium.reply(
        msg.from,
        `Sorry ${msg.sender.pushname}, you need to be administrator to do this.`,
        msg.id,
        true,
      );
    }

    /* if (this.props.missingArgs && (!args.length || (this.props.minArgs && args.length < this.props.minArgs))) {
      return Tritium.reply(msg.from, this.props.missingArgs, msg.id, true);
      //FIXME: wat to do ? return Tritium.reply(msg.from, this.getHelpMsg(Tritium.prefix), msg.id, true);
    } */

    // *** Arguments checks ***
    // *** Types: number of args, 'quotedMsg', ... ***
    if (this.props.minArgs && typeof this.props.minArgs === "string") {
      if (this.props.minArgs === "quotedMsg" && !msg.quotedMsg)
        return await Tritium.reply(msg.from, "*💭 You need to quote a message !*", msg.id);
